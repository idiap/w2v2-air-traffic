...
def cardinal_number(num_str, limit=99999):
    try:
        ...
        if num > limit: raise(Exception("number too big"))
        ...
        if (num < k):
            ...
            else: return d[num // 100] + ' hundred and ' + cardinal_number(num % 100)
        if (num < m):
            if num % k == 0: return cardinal_number(num // k) + ' thousand'
            else: return cardinal_number(num // k) + ' thousand ' + cardinal_number(num % k)

        raise (Exception("number too big"))

    except Exception as e:
        print("{}: WARNING, could not translate number:".format(__file__), num_str, str(e), file=sys.stderr)
        return num_str
...
def ordinal_number(n_str):
    ...
    try:
        ...
        else: return cardinal_number(n_str)
    except Exception as e:
        print("{}: WARNING, could not translate number:".format(__file__), n, str(e), file=sys.stderr)
        return n_str
